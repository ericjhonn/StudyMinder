...
{
    public partial class Revisoes42ViewModel : BaseViewModel
    {
        ...
        private async Task CarregarDadosIniciaisAsync()
        {
            ...
            catch (Exception ex)
            {
                ...
                {
                    _notificationService.ShowError("Erro ao Carregar", $"Erro ao carregar revisões: {ex.Message}");
                });
            }
            finally
            {
        ...
        private async Task CarregarRevisoesAsync()
        {
            ...
            try
            {
                var tipos42 = new List<TipoRevisaoEnum>
                {
                    TipoRevisaoEnum.Ciclo42
                };

                var resultado = await _revisaoService.ObterRevisoesPendentesAsync(
                    tipos42, PaginaAtual, ItensPorPagina, SearchText);
        ...
        {
            _cacheRevisoes = null;
            // Recarregar apenas se estamos na primeira página
            if (PaginaAtual == 1)
            {
                _ = Task.Run(async () => await CarregarRevisoesAsync());
            }
